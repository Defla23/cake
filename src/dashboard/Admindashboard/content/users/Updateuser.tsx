...
// Correct schema that matches UpdateUserForm exactly
const schema = yup.object({
  ...
  is_verified: yup.boolean().required().default(false),
});

export const UpdateUserModal = ({ selectedUser, refetchUsers }: UpdateUserModalProps) => {
  const [updateUser, { isLoading }] = adminUsersAPI.useUpdateUserMutation();
  ...
  // Reset form when modal is opened with a user
  useEffect(() => {
    if (selectedUser) {
      reset({
        name: selectedUser.name,
        email: selectedUser.email,
        role: selectedUser.role === "admin" ? "admin" : "customer",
      ...
      <div className="modal-box bg-gray-900 text-white w-full max-w-xs sm:max-w-lg mx-auto rounded-lg">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-4">
          ...
          <input
            {...register("name")}
            placeholder="Name"
            className="input rounded w-full p-2 bg-white text-black"
          />
          ...
          <select {...register("role")} className="input rounded w-full p-2 bg-white text-black">
          ...
          <div className="flex gap-4 items-center">
            ...
            <input
              ...
              className="checkbox checkbox-success bg-green-400"
            />
          </div>
          ...
          <div className="modal-action">
            <button
              type="submit"
              className="btn btn-primary bg-cyan-400"
              disabled={isLoading}
            >
